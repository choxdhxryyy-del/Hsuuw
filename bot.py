...
# ===== Text handler =====
@bot.message_handler(func=lambda message: user_state.get(message.chat.id) != "waiting_for_location" and not message.text.startswith('/'))
def chat(message):
    chat_id = message.chat.id
...
def handle_sticker(message):
    ...
    # Naya: Ab Groq AI khud sticker ke liye reply dega
    try:
        response = client.chat.completions.create(
            model="llama-3.1-8b-instant",
            messages=[
                {"role": "system", "content": "You are a professional AI assistant who replies in a very short, funny, and friendly way to a sticker. Reply in simple, colloquial Hindi with emojis. For example: 'Haha, bahut sahi sticker! 😂', 'Ekdum mast! 🤩', 'Kamaal kar diya! 👏'"},
